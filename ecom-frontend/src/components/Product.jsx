...
const Product = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/api/product/${id}`         // Requesting for the Product here
        );
        setProduct(response.data);
        if (response.data.imageName) {
          fetchImage();                               // Calling the Fetch Image Method
        }
      } catch (error) {
    ...
    const fetchImage = async () => {
      const response = await axios.get(
        `http://localhost:8080/api/product/${id}/image`,    // Requesting for the Product Image Here
        { responseType: "blob" }
      );
